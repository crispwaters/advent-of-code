...
const doTurnOn = (state: State): void => doAction(state, (brightness: number) => brightness + 1)
const doTurnOff = (state: State): void => doAction(state, (brightness: number) => Math.max(0, brightness - 1))
const doToggle = (state: State): void => doAction(state, (brightness: number) => brightness + 2)
const doAction = ({instruction, grid}: State, action: (brightness: number) => number): void => {
    const [min, max] = getRange(instruction);
    for (let x = min.x; x <= max.x; x++) {
        for (let y = min.y; y <= max.y; y++) {
            const key = `${x},${y}`;
            grid[key] = action(grid[key] || 0);
        }
    }
};
